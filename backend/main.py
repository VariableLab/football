...
# 枚举值验证集合
_VALID_STATUSES = {e.value for e in MatchStatus}
...
def _enrich_rationale(pick, match) -> str:
    ...
    from strategy_pipeline import OptimalPick
...
# ────────────────────────────
# Strategy (paywalled — free after match finishes)
# ────────────────────────────
from datetime import datetime, timezone as _tz

@app.get("/api/matches/{match_id}/strategy", response_model=StrategyResponse)
def get_strategy(
    match_id: int,
...
# ────────────────────────────
# Odds Movement (Public)
# ────────────────────────────
@app.get("/api/matches/{match_id}/odds-movement", response_model=OddsMovementResponse)
...
def list_feedback(
    ...
    from models import Feedback, FeedbackLike, User
...
@app.post("/api/feedback/{feedback_id}/like", response_model=FeedbackLikeResponse)
def like_feedback(
    feedback_id: int,
    user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db),
):
    from models import Feedback, FeedbackLike
...
# ────────────────────────────
@app.post("/api/sporttery/sync")
def sporttery_sync(days_ahead: int = 3, user: User = Depends(get_current_active_user)):
    """手动触发 sporttery.cn 数据同步"""
    from sporttery_sync import sync_from_sporttery
    result = sync_from_sporttery(days_ahead=days_ahead, generate_predictions=True)
    return {
        "status": "completed",
        "matches_created": result.matches_created,
        "matches_updated": result.matches_updated,
        "odds_written": result.odds_written,
        "opening_set": result.opening_set,
...
@app.post("/api/sporttery/backfill")
def sporttery_backfill(days_back: int = 7, user: User = Depends(get_current_active_user)):
    """手动触发历史数据回填"""
    from sporttery_sync import backfill_historical_issues
    result = backfill_historical_issues(days_back=days_back)
    return {
        "status": "completed",
        "matches_created": result.matches_created,
        "matches_updated": result.matches_updated,
...
@app.post("/api/sporttery/lock-closing")
def sporttery_lock_closing(user: User = Depends(get_current_active_user)):
    """手动锁定已结束比赛的收盘价"""
    from sporttery_sync import lock_closing_odds
    locked = lock_closing_odds()
    return {"status": "completed", "locked": locked}


@app.get("/api/sporttery/status")
def sporttery_status(db: Session = Depends(get_db)):
    """查看 sporttery.cn 数据同步状态"""
    total_matches = db.query(Match).filter(Match.odds_source == "sporttery").count()
    total_opening = db.query(Match).filter(Match.opening_odds_source == "sporttery").count()
    total_closing = db.query(Match).filter(Match.closing_odds_source == "sporttery").count()
    total_issues = db.query(JingcaiIssue).count()
    total_issue_matches = db.query(JingcaiIssueMatch).count()

    # 最新同步时间（按 odds_history）
    latest = db.query(OddsHistory).filter(
        OddsHistory.source == "sporttery"
    ).order_by(OddsHistory.recorded_at.desc()).first()

    return {
        "matches_with_sporttery_odds": total_matches,
        "matches_with_sporttery_opening": total_opening,
        "matches_with_sporttery_closing": total_closing,
        "total_issues": total_issues,
        "total_issue_matches": total_issue_matches,
        "latest_sync_at": latest.recorded_at.isoformat() if latest else None,
    }
...
def get_strategy_params():
    """获取当前策略参数"""
    from strategy_config import load_params, StrategyParams
    params = load_params()
    return {k: getattr(params, k) for k in StrategyParams.__dataclass_fields__}


@app.patch("/api/strategy/params")
def update_strategy_params(
    user: User = Depends(get_current_active_user),
    **kwargs,
):
    """更新策略参数（需管理员权限）"""
    from strategy_config import update_params
    new_params = update_params(**kwargs)
    return {k: getattr(new_params, k) for k in new_params.__dataclass_fields__}


@app.get("/api/predictions/{match_id}/tier")
def get_match_tier(match_id: int, db: Session = Depends(get_db)):
    """获取单场比赛的分层策略分析"""
    from tiered_strategy import analyze_match_from_db, tier_result_to_dict
    result = analyze_match_from_db(match_id)
    if not result:
        raise HTTPException(status_code=404, detail="比赛不存在或模型未就绪")
    return tier_result_to_dict(result)
