...
s = SessionLocal()
remaining = s.query(Prediction).filter(Prediction.confidence.is_(None)).count()
...
BATCH = 5000
while True:
    batch = s.query(Prediction).filter(Prediction.confidence.is_(None)).limit(BATCH).all()
    if not batch:
        break
    for p in batch:
        ...
        updated += 1
    s.commit()
    print(f"  进度: {updated}/{remaining}  high={stats['high']} medium={stats['medium']} low={stats['low']}")
